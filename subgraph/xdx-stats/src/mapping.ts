...
import {
  ...
  AddLiquidity,
  RemoveLiquidity,
} from "../generated/XLXManager/XLXManager";

// import {
//   Distribute
// } from "../generated/FeeXDXRewardDistributor/RewardDistributor"
...
function _getOrCreateXDXStat(id: string, period: string): XDXStat {
  let entity = XDXStat.load(id);
  if (entity == null) {
    entity = new XDXStat(id);
    ...
    entity.distributedEsXDX = ZERO;
    entity.distributedEsXDXCumulative = ZERO;
    entity.distributedEsXDXUsd = ZERO;
    entity.distributedEsXDXUsdCumulative = ZERO;
    entity.period = period;
  }
  return entity as XDXStat;
}
...
function _getOrCreateXLXStat(
  id: string,
  period: string,
  periodTimestmap: i32
): XLXStat {
  let entity = XLXStat.load(id);
  if (entity == null) {
    entity = new XLXStat(id);
    entity.period = period;
    entity.xlxSupply = ZERO;
  ...
  return entity as XLXStat;
}

function _storeXLXStat(
  timestamp: BigInt,
  xlxSupply: BigInt,
  aumInUsdg: BigInt
): void {
  let periodTimestamp = parseInt(_getDayId(timestamp)) as i32;
  let totalEntity = _getOrCreateXLXStat("total", "total", periodTimestamp);
  totalEntity.aumInUsdg = aumInUsdg;
  totalEntity.xlxSupply = xlxSupply;
  totalEntity.save();

  let id = periodTimestamp.toString() + ":daily";
  let entity = _getOrCreateXLXStat(id, "daily", periodTimestamp);
  entity.aumInUsdg = aumInUsdg;
  entity.xlxSupply = xlxSupply;
  entity.save();
}
