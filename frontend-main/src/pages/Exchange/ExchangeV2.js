...
import BooksAndTrades from "components/Exchange/BooksAndTrades";
...
export const Exchange = forwardRef((props, ref) => {
  ...
  const getListSection = () => {
    return (
      <div>
        <div className="grid grid-cols-[auto_auto] px-2 pt-2">
          <Tab
  ...
  return (
    <div className="m-auto mx-auto flex w-full flex-col justify-between">
      ...
      <div className="grid min-h-[calc(100vh-102px)] grid-cols-1 p-0 md:grid-cols-2 xl:grid-cols-[auto_1fr]">
        {account ? (
          <div
            className={cx("order-2 border-l border-slate-800 py-2 md:border-b xl:border-b-0", {
              "col-span-2": !active,
            })}
          >
            <SwapBox
          ...
          <div className="relative order-2 flex h-full flex-col justify-center overflow-y-auto border-r border-slate-800 px-4 py-2 md:border-b xl:w-85 xl:border-b-0">
            <div className="text-center">
              <p className="mt-4 text-sm text-slate-600">Please connect your wallet to start trading.</p>
              <div className="mt-6 items-center pl-8 pr-8 text-center">
                <button
                  type="button"
                  onClick={connectWallet}
                  className="inline-flex items-center rounded border border-transparent bg-indigo-500 px-4 py-2 text-center text-xs font-medium text-slate-200 shadow-sm hover:bg-indigo-600 hover:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-800"
                >
        ...
        {account && <BooksAndTrades indexToken={toTokenAddress} className="order-2 xl:order-3 xl:hidden" />}
        <div className="order-1 flex flex-1 flex-row flex-wrap items-start justify-start overflow-hidden overflow-y-auto md:col-span-2 xl:order-3 xl:col-span-1">
          <div className={cx("h-[496px] w-full", active ? "border-l border-slate-800 xl:w-[calc(100%-224px)]" : "")}>
            {renderChart()}
          </div>
          {account && <BooksAndTrades indexToken={toTokenAddress} className="hidden xl:flex" />}
          <div className="hidden min-h-[calc(100%-496px)] w-full lg:col-span-2 xl:block">{getListSection()}</div>
        </div>
        <div className="order-last w-full md:col-span-2 xl:col-span-1 xl:hidden">{getListSection()}</div>
      </div>
    </div>
  );
});
