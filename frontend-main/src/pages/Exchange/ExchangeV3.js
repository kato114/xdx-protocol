...
import SwapBox from "components/Exchange/SwapBoxV3";
import ExchangeTVChartBasic, { getChartToken as getChartTokenBasic } from "components/Exchange/ExchangeTVChartBasic";
import ExchangeTVChartPro, { getChartToken as getChartTokenPro } from "components/Exchange/ExchangeTVChartPro";
...
import { CHART_SELECTED_TAB_KEY } from "config/localStorage";
...
export const Exchange = forwardRef((props, ref) => {
  ...
  const [activeTab, setActiveTab] = useLocalStorageSerializeKey(CHART_SELECTED_TAB_KEY, BASIC);
  ...
  useEffect(() => {
    ...
    let selectedToken =
      activeTab === PRO
        ? getChartTokenPro(swapOption, fromToken, toToken, chainId)
        : getChartTokenBasic(swapOption, fromToken, toToken, chainId);
  ...
  }, [tokenSelection, swapOption, infoTokens, chainId, fromTokenAddress, toTokenAddress, activeTab]);
  ...
  //   return (
  //     <button
  //       className="font-base cancel-order-btn opacity-70"
  ...
  const getListSection = () => {
    return (
      <div className="md:px-4">
        <div className="grid grid-cols-[auto_auto] pt-2">
          <div className="w-80">
            <Tab
        ...
        <div className="w-full border-t border-slate-800 py-4">
  ...
  return (
    <div className="m-auto mx-auto flex w-full flex-col justify-between">
      {showBanner && <ExchangeBanner hideBanner={hideBanner} />}
      <div className="grid min-h-[calc(100vh-102px)] grid-cols-1 p-0 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6">
        <div className={cx("order-2 border-l border-slate-800 md:border-b xl:border-b-0")}>
          <SwapBox
            pendingPositions={pendingPositions}
            setPendingPositions={setPendingPositions}
            setIsWaitingForPluginApproval={setIsWaitingForPluginApproval}
            ...
            minExecutionFeeErrorMessage={minExecutionFeeErrorMessage}
          />
          <div className="relative hidden min-h-[155px]">
            <div className="absolute top-[15px] bottom-0 left-0 right-0">
              <ExchangeWalletTokens tokens={tokens} infoTokens={infoTokens} onSelectToken={onSelectWalletToken} />
            </div>
          </div>
        </div>
        {/* {account && (
          <div className="grid order-3 grid-cols-2 col-span-5 gap-0 xl:hidden">
            <Orderbook />
            <TradeList />
          </div>
        )} */}
        <div className="order-1 flex flex-1 flex-col items-start justify-start overflow-auto overflow-y-auto md:col-span-2 md:min-h-[calc(100vh-59px)] lg:col-span-3 xl:col-span-4 2xl:col-span-5">
          <div className={cx("h-[520px] w-full md:h-[594px] 2xl:h-[650px]")}>{renderChart()}</div>
          {/* <div className="hidden h-[364px] w-full grid-cols-2 gap-0 border-b border-slate-800 md:grid xl:hidden">
            <Orderbook toTokenAddress={toTokenAddress} infoTokens={infoTokens} />
            <TradeList toTokenAddress={toTokenAddress} infoTokens={infoTokens} />
          </div> */}
          <div className="col-span-4 hidden h-full max-h-[400px] min-h-[200px] w-full md:block">{getListSection()}</div>
        </div>
        <div className="order-last w-full md:col-span-2 md:hidden xl:col-span-1">{getListSection()}</div>
      </div>
    </div>
  );
});
