...
export async function getReferralCodeTakenStatus(account, referralCode, chainId) {
  ...
  const takenOnArb = !isAddressZero(ownerArbitrum) && (ownerArbitrum !== account || true);
  ...
  if (referralCodeTakenInfo[AVALANCHE]) {
...
export function getTwitterShareUrl(referralCode) {
  const message = "Trying out trading on @XDX_IO, up to 30x leverage on $BTC, $ETH 📈%0a%0aFor fee discounts use:";

  return `http://twitter.com/intent/tweet?text=${message}&url=https://xdx.io?ref=${referralCode}`;
}
