...
function JoinReferralCode({ setPendingTxns, pendingTxns, active, connectWallet }) {
  return (
    <div className="relative mx-auto mt-[15px] max-w-[475px] rounded border border-slate-800 p-[23.25px] text-center shadow bg-slate-950 bg-opacity-50">
      <h2 className="mb-[8px] gap-2 text-center text-lg font-medium text-slate-300">
        <Trans>Enter Referral Code</Trans>
      </h2>
      <p className="mt-[11.625px] text-xs text-slate-600 font-medium">
      ...
      <div className="mt-[31px]">
        {active ? (
          ...
          <button
            className="flex items-center justify-center w-full p-3 text-xs text-center bg-indigo-500 rounded cursor-pointer text-slate-200 disabled:cursor-not-allowed disabled:bg-slate-800 disabled:text-slate-200 hover:bg-indigo-600"
            type="button"
