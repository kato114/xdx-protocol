...
import { TVProChartContainer } from "./chart/TVProChartContainer";
...
export default function ExchangeTVChart(props) {
  ...
  useEffect(() => {
    ...
    const toSymbol = symbol ? `Bitfinex:${symbol}/USD` : "Bitfinex:BTC/USD";
  ...
  const tradingView = useMemo(() => {
    return (
      <TVProChartContainer
        symbol={tradingViewSetting.symbol}
        locale={tradingViewSetting.locale}
        interval="1"
        currentOrders={savedShouldShowPositionLines ? currentOrders : []}
        positions={savedShouldShowPositionLines ? positions : []}
        chainId={chainId}
      />
    );
  }, [
    tradingViewSetting.symbol,
    tradingViewSetting.locale,
    currentOrders,
    positions,
    savedShouldShowPositionLines,
    chainId,
