  ...
  return (
    <div className="TradeHistory">
      {tradesWithMessages.length === 0 && (
        <div className="relative border border-slate-800 p-[10.5px] px-[15px] text-center text-sm text-slate-600">
          No trades yet
        </div>
      )}
      {tradesWithMessages.length > 0 &&
        tradesWithMessages.map((trade, index) => {
          ...
          return (
            <div className="App-box-border relative mb-2 border-y border-slate-800 p-4 text-sm" key={index}>
              <div>
                <div className="TradeHistory-time opacity-70">
                ...
                <a className="no-underline" href={txUrl} target="_blank" rel="noopener noreferrer">
                  {msg}
                </a>
              </div>
            </div>
