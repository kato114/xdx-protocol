...
export default function ConfirmationBox(props) {
  ...
  const renderSpreadWarning = useCallback(() => {
    ...
    if (spread && spread.isHigh) {
      return (
        <div className="mt-[10px] mb-[15px] px-[10px] text-center text-sm font-medium">
      ...
      return (
        <div>
          <div className="mt-[10px] mb-[15px] px-[10px] text-center text-sm font-medium">
            ...
            <span onClick={() => setIsLimitOrdersVisible((p) => !p)} className="cursor-pointer no-underline">
            ...
            <ul className="m-0">
              {existingOrders.map((order) => {
                ...
                return (
                  <li
                    key={id}
                    className="font-sm mb-[5px] flex items-center justify-between rounded bg-slate-900 p-[10px]"
                  >
                    <p>
                    ...
                    <button
                      className="text-[14px] text-white opacity-80 hover:opacity-100"
      ...
      <>
        ...
        <ul className="m-0">
          {decreaseOrdersThatWillBeExecuted.map((order) => {
            ...
            const indexToken = getToken(chainId, order.indexToken);
            return (
              <li key={id} className="font-sm mb-[5px] flex items-center justify-between rounded bg-slate-900 p-[10px]">
                <p>
                  {type === INCREASE ? "Increase" : "Decrease"} {indexToken.symbol} {isLong ? "Long" : "Short"}
                  &nbsp;{triggerPricePrefix} ${formatAmount(triggerPrice, USD_DECIMALS, 2, true)}
                </p>
    ...
    return (
      <ExchangeInfoRow label="Available Liquidity">
        <Tooltip
          ...
          handleClassName={isLiquidityRisk ? "text-[#fa3c58]" : null}
          handle={
            <>
      ...
      <>
        <div>
          ...
          {hasPendingProfit && isMarketOrder && (
            <div className="rtl-grid mb-1 grid w-full grid-cols-[1fr_auto] text-[14px]">
          ...
          {isMarketOrder && (
            <div className="PositionEditor-allow-higher-slippage">
              ...
              >
                <span className="text-xs font-medium text-slate-600">Allow up to 1% slippage</span>
              </Checkbox>
            </div>
          )}
          {decreaseOrdersThatWillBeExecuted.length > 0 && (
            <div className="PositionEditor-allow-higher-slippage">
              <Checkbox isChecked={isTriggerWarningAccepted} setIsChecked={setIsTriggerWarningAccepted}>
                <span className="text-xs font-medium text-slate-600">I am aware of the trigger orders</span>
              </Checkbox>
            </div>
          )}
          {renderExecutionFee()}
    ...
    return (
      <>
        <div>
          ...
          {!isMarketOrder && (
            <div className="mb-[4.65px] grid grid-cols-[auto_auto] text-xs font-medium text-slate-300">
              <div className="mr-2 text-xs font-medium text-slate-600">Limit Price</div>
          ...
          <div className="mb-[4.65px] grid grid-cols-[auto_auto] text-xs font-medium text-slate-200">
            <div className="mr-2 text-xs font-medium text-slate-600">Fees</div>
            <div className="flex items-end justify-end text-end text-xs font-medium text-slate-300">
          ...
          {fromTokenUsd && (
            <div className="mb-[4.65px] grid grid-cols-[auto_auto] text-xs font-medium text-slate-200">
              <div className="mr-2 text-xs font-medium text-slate-600">{fromTokenInfo.symbol} Price</div>
              <div className="flex items-end justify-end text-end">{fromTokenUsd} USD</div>
            </div>
          )}
          {toTokenUsd && (
            <div className="mb-[4.65px] grid grid-cols-[auto_auto] text-xs font-medium text-slate-200">
              <div className="mr-2 text-xs font-medium text-slate-600">{toTokenInfo.symbol} Price</div>
              <div className="flex items-end justify-end text-end">{toTokenUsd} USD</div>
            </div>
          )}
        </div>
      </>
      ...
      >
        ...
        <div className="mt-[15px]">
          <button
            ...
            className="inline-block w-full cursor-pointer rounded bg-slate-800 p-3 text-center text-sm text-slate-300 hover:bg-indigo-500 disabled:cursor-not-allowed"
