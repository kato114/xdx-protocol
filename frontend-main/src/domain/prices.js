...
import { XDX_STATS_API_URL } from "config/backend";
...
async function getChartPricesFromStats(chainId, symbol, period) {
  ...
  const url = `${XDX_STATS_API_URL}/candles/${symbol}?preferableChainId=${chainId}&period=${period}&from=${from}&preferableSource=fast`;
