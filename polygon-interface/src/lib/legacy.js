...
export const XLX_DECIMALS = 18;
export const XDX_DECIMALS = 18;
...
export const XLX_COOLDOWN_DURATION = 15 * 60;
...
export function getBuyXLXToAmount(fromAmount, swapTokenAddress, infoTokens, xlxPrice, usdgSupply, totalTokenWeights) {
  const defaultValue = { amount: bigNumberify(0), feeBasisPoints: 0 };
  if (!fromAmount || !swapTokenAddress || !infoTokens || !xlxPrice || !usdgSupply || !totalTokenWeights) {
    return defaultValue;
  }
  ...
  let xlxAmount = fromAmount.mul(swapToken.minPrice).div(xlxPrice);
  xlxAmount = adjustForDecimals(xlxAmount, swapToken.decimals, USDG_DECIMALS);
  ...
  xlxAmount = xlxAmount.mul(BASIS_POINTS_DIVISOR - feeBasisPoints).div(BASIS_POINTS_DIVISOR);

  return { amount: xlxAmount, feeBasisPoints };
}

export function getSellXLXFromAmount(toAmount, swapTokenAddress, infoTokens, xlxPrice, usdgSupply, totalTokenWeights) {
  const defaultValue = { amount: bigNumberify(0), feeBasisPoints: 0 };
  if (!toAmount || !swapTokenAddress || !infoTokens || !xlxPrice || !usdgSupply || !totalTokenWeights) {
    return defaultValue;
  }
  ...
  let xlxAmount = toAmount.mul(swapToken.maxPrice).div(xlxPrice);
  xlxAmount = adjustForDecimals(xlxAmount, swapToken.decimals, USDG_DECIMALS);
  ...
  xlxAmount = xlxAmount.mul(BASIS_POINTS_DIVISOR).div(BASIS_POINTS_DIVISOR - feeBasisPoints);

  return { amount: xlxAmount, feeBasisPoints };
}

export function getBuyXLXFromAmount(toAmount, fromTokenAddress, infoTokens, xlxPrice, usdgSupply, totalTokenWeights) {
  const defaultValue = { amount: bigNumberify(0) };
  if (!toAmount || !fromTokenAddress || !infoTokens || !xlxPrice || !usdgSupply || !totalTokenWeights) {
    return defaultValue;
  }
  ...
  let fromAmount = toAmount.mul(xlxPrice).div(fromToken.minPrice);
  fromAmount = adjustForDecimals(fromAmount, XLX_DECIMALS, fromToken.decimals);

  const usdgAmount = toAmount.mul(xlxPrice).div(PRECISION);
  const feeBasisPoints = getFeeBasisPoints(
    fromToken,
    usdgAmount,
    MINT_BURN_FEE_BASIS_POINTS,
...
export function getSellXLXToAmount(toAmount, fromTokenAddress, infoTokens, xlxPrice, usdgSupply, totalTokenWeights) {
  const defaultValue = { amount: bigNumberify(0) };
  if (!toAmount || !fromTokenAddress || !infoTokens || !xlxPrice || !usdgSupply || !totalTokenWeights) {
    return defaultValue;
  }
  ...
  let fromAmount = toAmount.mul(xlxPrice).div(fromToken.maxPrice);
  fromAmount = adjustForDecimals(fromAmount, XLX_DECIMALS, fromToken.decimals);
...
export function getBalanceAndSupplyData(balances) {
  ...
  const keys = ["xdx", "esXDX", "xlx", "stakedXDXTracker"];
  const balanceData = {};
  const supplyData = {};
  const propsLength = 2;
...
export function getStakingData(stakingInfo) {
  ...
  const keys = ["stakedXDXTracker", "bonusXDXTracker", "feeXDXTracker", "stakedXLXTracker", "feeXLXTracker"];
  const data = {};
  const propsLength = 5;
...
export function getProcessedData(
  ...
  data.xdxBalance = balanceData.xdx;
  data.xdxBalanceUsd = balanceData.xdx.mul(xdxPrice).div(expandDecimals(1, 18));

  data.xdxSupply = bigNumberify(xdxSupply);

  data.xdxSupplyUsd = data.xdxSupply.mul(xdxPrice).div(expandDecimals(1, 18));
  data.stakedXDXSupply = stakedXDXSupply;
  data.stakedXDXSupplyUsd = stakedXDXSupply.mul(xdxPrice).div(expandDecimals(1, 18));
  data.xdxInStakedXDX = depositBalanceData.xdxInStakedXDX;
  data.xdxInStakedXDXUsd = depositBalanceData.xdxInStakedXDX.mul(xdxPrice).div(expandDecimals(1, 18));

  data.esXDXBalance = balanceData.esXDX;
  data.esXDXBalanceUsd = balanceData.esXDX.mul(xdxPrice).div(expandDecimals(1, 18));

  data.stakedXDXTrackerSupply = supplyData.stakedXDXTracker;
  data.stakedXDXTrackerSupplyUsd = supplyData.stakedXDXTracker.mul(xdxPrice).div(expandDecimals(1, 18));
  data.stakedEsXDXSupply = data.stakedXDXTrackerSupply.sub(data.stakedXDXSupply);
  data.stakedEsXDXSupplyUsd = data.stakedEsXDXSupply.mul(xdxPrice).div(expandDecimals(1, 18));

  data.esXDXInStakedXDX = depositBalanceData.esXDXInStakedXDX;
  data.esXDXInStakedXDXUsd = depositBalanceData.esXDXInStakedXDX.mul(xdxPrice).div(expandDecimals(1, 18));

  data.bnXDXInFeeXDX = depositBalanceData.bnXDXInFeeXDX;
  data.bonusXDXInFeeXDX = depositBalanceData.bonusXDXInFeeXDX;
  data.feeXDXSupply = stakingData.feeXDXTracker.totalSupply;
  data.feeXDXSupplyUsd = data.feeXDXSupply.mul(xdxPrice).div(expandDecimals(1, 18));

  data.stakedXDXTrackerRewards = stakingData.stakedXDXTracker.claimable;
  data.stakedXDXTrackerRewardsUsd = stakingData.stakedXDXTracker.claimable.mul(xdxPrice).div(expandDecimals(1, 18));

  data.bonusXDXTrackerRewards = stakingData.bonusXDXTracker.claimable;

  data.feeXDXTrackerRewards = stakingData.feeXDXTracker.claimable;
  data.feeXDXTrackerRewardsUsd = stakingData.feeXDXTracker.claimable.mul(nativeTokenPrice).div(expandDecimals(1, 18));

  data.boostBasisPoints = bigNumberify(0);
  if (data && data.bnXDXInFeeXDX && data.bonusXDXInFeeXDX && data.bonusXDXInFeeXDX.gt(0)) {
    data.boostBasisPoints = data.bnXDXInFeeXDX.mul(BASIS_POINTS_DIVISOR).div(data.bonusXDXInFeeXDX);
  }

  data.stakedXDXTrackerAnnualRewardsUsd = stakingData.stakedXDXTracker.tokensPerInterval
    .mul(SECONDS_PER_YEAR)
    .mul(xdxPrice)
    .div(expandDecimals(1, 18));
  data.xdxAprForEsXDX =
    data.stakedXDXTrackerSupplyUsd && data.stakedXDXTrackerSupplyUsd.gt(0)
      ? data.stakedXDXTrackerAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.stakedXDXTrackerSupplyUsd)
      : bigNumberify(0);
  data.feeXDXTrackerAnnualRewardsUsd = stakingData.feeXDXTracker.tokensPerInterval
    .mul(SECONDS_PER_YEAR)
    .mul(nativeTokenPrice)
    .div(expandDecimals(1, 18));
  data.xdxAprForNativeToken =
    data.feeXDXSupplyUsd && data.feeXDXSupplyUsd.gt(0)
      ? data.feeXDXTrackerAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.feeXDXSupplyUsd)
      : bigNumberify(0);
  data.xdxBoostAprForNativeToken = data.xdxAprForNativeToken.mul(data.boostBasisPoints).div(BASIS_POINTS_DIVISOR);
  data.xdxAprTotal = data.xdxAprForNativeToken.add(data.xdxAprForEsXDX);
  data.xdxAprTotalWithBoost = data.xdxAprForNativeToken.add(data.xdxBoostAprForNativeToken).add(data.xdxAprForEsXDX);
  data.xdxAprForNativeTokenWithBoost = data.xdxAprForNativeToken.add(data.xdxBoostAprForNativeToken);

  data.totalXDXRewardsUsd = data.stakedXDXTrackerRewardsUsd.add(data.feeXDXTrackerRewardsUsd);

  data.xlxSupply = supplyData.xlx;
  data.xlxPrice =
    data.xlxSupply && data.xlxSupply.gt(0)
      ? aum.mul(expandDecimals(1, XLX_DECIMALS)).div(data.xlxSupply)
      : bigNumberify(0);

  data.xlxSupplyUsd = supplyData.xlx.mul(data.xlxPrice).div(expandDecimals(1, 18));

  data.xlxBalance = depositBalanceData.xlxInStakedXLX;
  data.xlxBalanceUsd = depositBalanceData.xlxInStakedXLX.mul(data.xlxPrice).div(expandDecimals(1, XLX_DECIMALS));

  data.stakedXLXTrackerRewards = stakingData.stakedXLXTracker.claimable;
  data.stakedXLXTrackerRewardsUsd = stakingData.stakedXLXTracker.claimable.mul(xdxPrice).div(expandDecimals(1, 18));

  data.feeXLXTrackerRewards = stakingData.feeXLXTracker.claimable;
  data.feeXLXTrackerRewardsUsd = stakingData.feeXLXTracker.claimable.mul(nativeTokenPrice).div(expandDecimals(1, 18));

  data.stakedXLXTrackerAnnualRewardsUsd = stakingData.stakedXLXTracker.tokensPerInterval
    .mul(SECONDS_PER_YEAR)
    .mul(xdxPrice)
    .div(expandDecimals(1, 18));
  data.xlxAprForEsXDX =
    data.xlxSupplyUsd && data.xlxSupplyUsd.gt(0)
      ? data.stakedXLXTrackerAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.xlxSupplyUsd)
      : bigNumberify(0);
  data.feeXLXTrackerAnnualRewardsUsd = stakingData.feeXLXTracker.tokensPerInterval
    .mul(SECONDS_PER_YEAR)
    .mul(nativeTokenPrice)
    .div(expandDecimals(1, 18));
  data.xlxAprForNativeToken =
    data.xlxSupplyUsd && data.xlxSupplyUsd.gt(0)
      ? data.feeXLXTrackerAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.xlxSupplyUsd)
      : bigNumberify(0);
  data.xlxAprTotal = data.xlxAprForNativeToken.add(data.xlxAprForEsXDX);

  data.totalXLXRewardsUsd = data.stakedXLXTrackerRewardsUsd.add(data.feeXLXTrackerRewardsUsd);

  data.totalEsXDXRewards = data.stakedXDXTrackerRewards.add(data.stakedXLXTrackerRewards);
  data.totalEsXDXRewardsUsd = data.stakedXDXTrackerRewardsUsd.add(data.stakedXLXTrackerRewardsUsd);

  data.xdxVesterRewards = vestingData.xdxVester.claimable;
  data.xlxVesterRewards = vestingData.xlxVester.claimable;
  data.totalVesterRewards = data.xdxVesterRewards.add(data.xlxVesterRewards);
  data.totalVesterRewardsUsd = data.totalVesterRewards.mul(xdxPrice).div(expandDecimals(1, 18));

  data.totalNativeTokenRewards = data.feeXDXTrackerRewards.add(data.feeXLXTrackerRewards);
  data.totalNativeTokenRewardsUsd = data.feeXDXTrackerRewardsUsd.add(data.feeXLXTrackerRewardsUsd);

  data.totalRewardsUsd = data.totalEsXDXRewardsUsd.add(data.totalNativeTokenRewardsUsd).add(data.totalVesterRewardsUsd);
...
export function isDevelopment() {
  return !window.location.host?.includes("xdx.io") && !window.location.host?.includes("ipfs.io");
}
...
export function getRootShareApiUrl() {
  if (isLocal()) {
    return "https://xdxs.vercel.app";
  }

  return "https://share.xdx.io";
}
...
export function getTradePageUrl() {
  ...
  return "https://app.xdx.exchange/#/trade";
}
