...
import { ARBITRUM, AVALANCHE, getConstant, getHighExecutionFee } from "config/chains";
import { DECREASE, getOrderKey, INCREASE, SWAP, USD_DECIMALS } from "lib/legacy";

import { groupBy } from "lodash";
import { UI_VERSION } from "config/ui";
import { getServerBaseUrl, getServerUrl } from "config/backend";
import { getXDXGraphClient, nissohGraphClient, avalancheGraphClientForTrades } from "lib/subgraph/clients";
...
export function useXDXPrice(chainId, libraries, active) {
  ...
  const xdxPrice = chainId === AVALANCHE ? xdxPriceFromAvalanche : xdxPriceFromArbitrum;
...
export function useTotalXDXSupply() {
  // const xdxSupplyUrlArbitrum = getServerUrl(ARBITRUM, "/xdx_supply");

  // const { data: xdxSupply, mutate: updateXDXSupply } = useSWR([xdxSupplyUrlArbitrum], {
  //   fetcher: (...args) => fetch(...args).then((res) => res.text()),
  // });
  ...
  // };
  const { data: xdxSupply } = useSWR(
    [`StakeV2:totalSupply:${AVALANCHE}`, AVALANCHE, getContract(AVALANCHE, "XDX"), "totalSupply"],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  );
...
export function useTotalXDXInLiquidity() {
  let poolAddressArbitrum = getContract(ARBITRUM, "TraderJoeXDXPool");
  let poolAddressAvax = getContract(AVALANCHE, "TraderJoeXDXPool");
  ...
  }, [mutateXDXInLiquidityOnArbitrum, mutateXDXInLiquidityOnAvax]);

  if (xdxInLiquidityOnAvax && xdxInLiquidityOnArbitrum) {
...
function useXDXPriceFromAvalanche() {
  const poolAddress = getContract(AVALANCHE, "TraderJoeXDXPool");
...
function useXDXPriceFromArbitrum(library, active) {
  const poolAddress = getContract(ARBITRUM, "TraderJoeXDXPool");
  ...
  const ethAddress = getTokenBySymbol(ARBITRUM, "WAVAX").address;
...
export function useFeesData({ from = FIRST_DATE_TS, to = NOW_TS } = {}) {
  ...
  useEffect(() => {
    const graphClient = getXDXGraphClient(AVALANCHE);
    if (!graphClient) {
      return;
    }
    graphClient.query({ query }).then(setRes).catch(console.warn);
